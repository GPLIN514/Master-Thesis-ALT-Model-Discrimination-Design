...
shiny_demo_Arrhenius_server <- function(input, output, session) {
  ...
  observeEvent(input$run_Arrhenius, {
    shinyjs::show("loading_spinner_Arrhenius")
    ...
    output$bestDesign_Arrhenius <- renderTable({
      colnames(best_design_Arrhenius) <- c("Support Point", "Weight")
    ...
    output$selectedSettings_Arrhenius <- renderTable({
      settings <- data.frame(
        Setting = c(
          ...
          "Rival Model Dispersion Parameter Bound"
        ),
        Value = c(
    ...
    shinyjs::hide("loading_spinner_Arrhenius")
  })
}